...
dept_short = {'Artificial Intelligence and Machine Learning': 'AIML',
              'Biotechnology': 'BT',
...
def print_faculty_wordxml(tt, designation, faculty_subjects, subs, sections):
    ...
    root[0][4][2][1].text = root[0][4][2][1].text.replace(
        'AIML', dept_short[tt.dept])  # branch
    ...
    if len(reg_subs) > 0:
        ...
        r[4][1].text = '{} - {} - {}'.format(reg_subs[0]
                                             [0], reg_subs[0][1], sections[sem][sec].roomno)
        next_sub = 9
        for short_sub, sec in reg_subs[1:]:
            p = copy.deepcopy(r)
            sem, s = sec.split(' ')
            p[4][1].text = '{} - {} - {}'.format(
                short_sub, sec, sections[sem][s].roomno)
...
def print_section_wordxml(tt, subjects_assigned, subs, year, faculty):
    ...
    root[0][3][16][1].text = root[0][3][16][1].text.replace(
        's00', tt.name)  # section
    root[0][1][1][1].text = 'Department of ' + tt.dept  # department - long
    root[0][4][17][1].text = root[0][4][17][1].text.replace('AIML', dept_short[tt.dept])
        ...
        if sub.lab == True:
            ...
            for i, lname in enumerate(lab):
                if lname.endswith(' Lab') == False:
                    lname = lname + ' Lab'
                initials = ''.join(
                    map(lambda x: x[0].upper(), fac[i].split(' ')))
                labs.append((lname, initials))
        else:
            regular_subs.append(
                (sub_name, sub.subcode, str(faculty[faculty.index(fac)])))
...
def make_docx(tt, tt_type, filename, subjects_assigned=None, subs=None, dicts=None, year=None):
    if tt_type == 'section':
        xml_string = print_section_wordxml(
            tt, subjects_assigned, subs, year, dicts)
    elif tt_type == 'faculty':
        xml_string = print_faculty_wordxml(
            tt, year, subjects_assigned, subs, dicts)
    else:
