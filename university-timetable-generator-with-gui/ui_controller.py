...
class subject:
    ...
    def __repr__(self):
        return "subject({}, {}, {}, {}, {})".format(
            self.name.__repr__(),
            self.short_name.__repr__(),
            self.credits,
            self.lab,
            self.subcode.__repr__(),
        )
...
class faculty_class:
    ...
    def __repr__(self):
        return "faculty_class({}, {}, {})".format(
            self.name.__repr__(), self.title.__repr__(), self.designation.__repr__()
        )
...
def transform_student_timetable_for_mongodb(tt, sem, sec, faculty_list_value):
    # Build a mapping from just the faculty name to full title name and designation
    faculty_lookup = {}
    for fac in faculty_list_value:
        full_name = f"{fac.title} {fac.name}"
        faculty_lookup[fac.name] = {
            "full_name": full_name,
            "designation": fac.designation
        }

    mongodb_documents = []
    for day_name, schedule_details in tt:
        day_document = {"day": day_name, "time_slots": []}

        slots = schedule_details.items()

        for slot_number, course_info in slots:
            slot_document = {"slot_number": slot_number}
            if isinstance(course_info, list) and len(course_info) == 4:
                faculty_name = course_info[2]
                faculty_info = faculty_lookup.get(
                    faculty_name, {"full_name": faculty_name, "designation": None})

                slot_document.update(
                    {
                        "is_empty": False,
                        "course_name": course_info[0],
                        "duration_or_credits": course_info[1],
                        "faculty": faculty_info["full_name"],
                ...
                slot_document.update(
                    {
                        "is_empty": True,
                        "raw_details": course_info,
                    }
                )
            day_document["time_slots"].append(slot_document)

        mongodb_documents.append(day_document)

    timetable = {"semester": sem, "section": sec,
                 "schedule": mongodb_documents}
    return timetable


def transform_faculty_timetable_for_mongodb(timetable_data, faculty_list_value):
    ...
    for day_schedule_tuple in timetable_data:
        ...
        # This can be an OrderedDict or a dict
        periods_data = day_schedule_tuple[1]
        ...
        factulty_timetable = {"faculty": faculty_info["full_name"], "designation": faculty_info["designation"],
                              "timetable": mongodb_documents}
    return factulty_timetable
...
class ParentWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        logger.info(
            "Current screen resolution: %s %s", self.screen_width, self.screen_height
        )
        ...
        self.mongo_uri = "mongodb://localhost:27017/"  # Your MongoDB URI
        self.mongo_db_name = "timetable_db"
        self.mongo_student_tt_collection_name = "student_timetables"
        self.mongo_faculty_tt_collection_name = "faculty_timetables"
        ...
        try:
            ...
            self.mongo_client.admin.command("ping")  # Verify connection
            self.db = self.mongo_client[self.mongo_db_name]
            logger.info("Successfully connected to MongoDB.")
        except pymongo.errors.ServerSelectionTimeoutError as err:
            logger.error(f"Failed to connect to MongoDB: {err}")
            # Optionally, disable publish button or show persistent error
            if hasattr(self, "systemtray_icon"):
            ...
            # Mock data (replace with your actual attributes)
            # These would be populated by other parts of your application
        self.timetables = {}  # e.g., {"SEM1": {"A": timetable_obj1}}
        self.faculty_timetables = {}  # e.g., {"Prof_X": faculty_tt_obj1}
        self.subjects_assigned = {}  # e.g., {"SEM1": {"A": subject_details}}
        self.faculty_subjects = {}  # e.g., {"Prof_X": subject_details}
        self.faculty_list_value = []  # List of faculty objects/dictionaries
        self.startMonth = "Nov"
        self.startYear = "2024"
        self.endMonth = "May"
        self.endYear = "2025"
        # self.systemtray_icon should be initialized by PyQt setup
        # global logger (or self.logger) should be initialized
    ...
    def setup_first_window(self):
        ...
        self.ui.sections_spinbox.valueChanged.connect(
            self.section_spinbox_event)
        ...
        self.ui.inputType_combobox.activated[str].connect(
            self.inputType_combobox_event)
        self.ui.semester_combobox.activated[str].connect(
            self.semester_combobox_event)
        ...
        self.FirstWindow.resize(
            int(self.screen_width * self.resize_ratio),
            int(self.screen_height * self.resize_ratio),
        )
        self.FirstWindow.updateGeometry()

        self.ui.menubar.triggered[QtWidgets.QAction].connect(
            self.filemenuevent)
    ...
    def setup_elective_window(self):
        ...
        self.ui_elec.semester_combobox.activated[str].connect(
            self.semester_combobox_elective_event
        )
        self.ui_elec.elective_spinbox.valueChanged.connect(
            self.elective_spinbox_event)
        self.ui_elec.elective_short_input.returnPressed.connect(
            self.ui_elec.addBtn.click
        )
        self.ui_elec.elective_input_textbox.returnPressed.connect(
            self.ui_elec.addBtn.click
        )
        ...
        self.ui_elec.elective_list.itemClicked.connect(
            self.handle_listclick_elective_event
        )
        self.ui_elec.removeBtn.clicked.connect(self.remove_btn_elective_event)
        self.ui_elec.electiveGroup_combobox.activated[str].connect(
            self.electivegroup_combobox_event
        )
    ...
    def setup_second_window(self):
        ...
        self.ui2.semester_combobox.activated[str].connect(
            self.semester_combobox2_event)
        self.ui2.assignBtn.clicked.connect(self.assign_btn_event)
        self.ui2.undoBtn.clicked.connect(self.undo_btn_event)
        self.ui2.section_combobox.activated[str].connect(
            self.section_combobox2_event)
        self.ui2.faculty_combobox.activated[str].connect(
            self.faculty_combobox2_event)

        self.sections = OrderedDict()
        self.subjects_assigned = (
            OrderedDict()
        )  # this dict will be like {'III': {'A': [subjects], 'B': [subjects]}, 'IV': {} ..etc}
        self.faculty_subjects = dict()  # stores subjects assigned to each faculty

        self.SecondWindow.resize(
            int(self.screen_width * self.resize_ratio),
            int(self.screen_height * self.resize_ratio),
        )
        self.SecondWindow.updateGeometry()

        self.ui2.menubar.triggered[QtWidgets.QAction].connect(
            self.filemenuevent)
    ...
    def setup_third_window(self):
        ...
        self.ui3.semester_combobox.activated[str].connect(
            self.semester_combobox3_event)
        self.ui3.section_combobox.activated[str].connect(
            self.section_combobox3_event)
        self.ui3.slotType_combobox.activated[str].connect(
            self.slotType_combobox3_event)
        ...
        self.ThirdWindow.resize(
            int(self.screen_width * self.resize_ratio),
            int(self.screen_height * self.resize_ratio),
        )
        self.ThirdWindow.updateGeometry()

        self.ui3.menubar.triggered[QtWidgets.QAction].connect(
            self.filemenuevent)
    ...
    def setup_fourth_window(self):
        ...
        self.ui4.faculty_combobox.activated[str].connect(
            self.faculty_combobox4_event)
        ...
        self.FourthWindow.resize(
            int(self.screen_width * self.resize_ratio),
            int(self.screen_height * self.resize_ratio),
        )
        self.FourthWindow.updateGeometry()

        self.ui4.menubar.triggered[QtWidgets.QAction].connect(
            self.filemenuevent)
    ...
    def setup_fifth_window(self):
        ...
        self.ui5.publishBtn.clicked.connect(self.publish_btn_event)
        self.ui5.printBtn.clicked.connect(self.print_btn_event)
        self.ui5.inputType_combobox.activated[str].connect(
            self.inputType_combobox_event
        )
        self.ui5.semester_combobox.activated[str].connect(
            self.semester_combobox5_event)
        self.ui5.section_combobox.activated[str].connect(
            self.section_combobox5_event)
        self.ui5.faculty_combobox.activated[str].connect(
            self.faculty_combobox5_event)
        ...
        self.ui5.roomno_textbox.returnPressed.connect(
            self.roomno_textbox_event)
        ...
        self.FifthWindow.resize(
            int(self.screen_width * self.resize_ratio),
            int(self.screen_height * self.resize_ratio),
        )
        self.FifthWindow.updateGeometry()

        self.ui5.menubar.triggered[QtWidgets.QAction].connect(
            self.filemenuevent)
    ...
    def setup_about_window(self):
        ...
        self.ui_about.logoLabel.setPixmap(QtGui.QPixmap("icons/rnsitlogo.png"))
    ...
    def setup_license_window(self):
        ...
        self.ui_license.logoLabel.setPixmap(
            QtGui.QPixmap("icons/rnsitlogo.png"))
    ...
    def continue_btn_event(self):
        ...
        logger.info(
            "Academic year: %s %s %s %s Department: %s",
            self.startMonth,
            self.startYear,
            self.endMonth,
            self.endYear,
    ...
    def reset_year_window(self):
        ...
        self.ui_year.startYear_dateedit.setDate(
            QtCore.QDate.fromString(str(self.startYear), "yyyy")
        )
        self.ui_year.endYear_dateedit.setDate(
            QtCore.QDate.fromString(str(self.endYear), "yyyy")
        )
    ...
    def inputType_combobox_event(
        self,
    ):  # function for input type combobox in first form
    ...
    def semester_combobox_event(self):  # function for semester combobox
        ...
        if self.sem != "" and self.sem not in self.num_sections:
            ...
            logger.info(
                "No. of sections in %s: %s", self.sem, self.num_sections[self.sem]
            )
    ...
    def section_spinbox_event(self):  # function for sections spinbox
        if self.sem != "":
            self.num_sections[self.sem] = (
                self.ui.sections_spinbox.value()
            )  # saves the number of sections when "add" button is clicked
                ...
                for x in self.row:
                    ...
                    for y in self.subjects[sem]:
                        if subj == y:
                            if not sem:
                                self.systemtray_icon.showMessage(
                                    "Input", "Please select the semester."
                                )
                                self.systemtray_icon.hide()
                                logger.debug(
                                    "Input-Please select the semester.")
                            ...
                            if text != "":
                                ...
                                if not short_sub:
                                    self.systemtray_icon.showMessage(
                                        "Input", "Please enter the subject short form."
                                    )
                                    self.systemtray_icon.hide()
                                    logger.debug(
                                        "Input-Please enter the subject short form."
                                ...
                                y.subcode = subcode
                                logger.info(
                                    "Subject details(name, shortname, credits, lab) :%s %s %s %s",
                                    text,
                                    short_sub,
                                    credits,
                                    lab,
                                ...
                                t = text + " - " + short_sub
                                self.ui.input_list.takeItem(
                                    self.ui.input_list.row(x))
                ...
                for x in self.row:
                    ...
                    for y in self.faculty_list_value:
                        if fac == y.name:
                            ...
                            logger.debug(
                                "Faculty details(title, name, designation): %s %s %s",
                                y.title,
                                y.name,
                                y.designation,
                            )
            ...
            if self.inputType == "Subjects":
                ...
                if text != "":
                    ...
                    for x in self.subjects[sem]:
                        if sub == x:
                            self.systemtray_icon.showMessage(
                                "Warning!", "Duplicate value entered."
                            )
                            self.systemtray_icon.hide()
                    ...
                    self.systemtray_icon.showMessage(
                        "Input", "Please enter the subject name."
                    )
                    self.systemtray_icon.hide()
                ...
                if text != "":
                    ...
                    if t not in self.faculty_list_value:
                        ...
                        self.ui.input_list.addItem(text)
                        logger.debug(
                            "Faculty details(title, name, designation): %s %s",
                            t,
                            f.designation,
                        )
                    else:
                    ...
                    self.systemtray_icon.showMessage(
                        "Input", "Please enter the faculty name."
                    )
                    self.systemtray_icon.hide()
                ...
                self.systemtray_icon.showMessage(
                    "Input", "Please select the input type."
                )
                self.systemtray_icon.hide()
                ...
                for section in self.section_fixed_slots[sem]:
                    for row in self.section_fixed_slots[sem][section]:
                        for col in self.section_fixed_slots[sem][section][row]:
                            if (
                                self.section_fixed_slots[sem][section][row][col]
                                == old_sub.short_name
                            ):
                                self.section_fixed_slots[sem][section][row][
                                    col
                                ] = new_sub.short_name
            ...
            for section in self.subjects_assigned[sem]:
                for i, sub in enumerate(self.subjects_assigned[sem][section]):
                    ...
                    if sub[0] == old_sub.name and sub[1] == old_sub.short_name:
                        self.subjects_assigned[sem][section][i] = (
                            new_sub.both_names + " - " + sub[2]
                        )
                        ...
                        for teacher in teachers:
                            ...
                            for i, sub in enumerate(self.faculty_subjects[teacher]):
                                ...
                                if (
                                    sub[0] == old_sub.name
                                    and sub[1] == old_sub.short_name
                                ):
                                    self.faculty_subjects[teacher][i] = (
                                        new_sub.both_names + " - " + sub[2]
                                    )
            ...
            for sem in self.subjects_assigned:
                for section in self.subjects_assigned[sem]:
                    for i, sub in enumerate(self.subjects_assigned[sem][section]):
                        ...
                        if sub[2] == old_fac:
                            self.subjects_assigned[sem][section][i] = (
                                " - ".join(sub[:2]) + " - " + new_fac.name
                            )

            if old_fac in self.faculty_subjects:
                self.faculty_subjects[new_fac] = self.faculty_subjects.pop(
                    old_fac)
            if old_fac in self.faculty_fixed_slots:
                self.faculty_fixed_slots[new_fac] = self.faculty_fixed_slots.pop(
                    old_fac
                )
        pass
        ...
        if not row:
            ...
            for x in row:
                ...
                try:
                    if self.inputType == "Subjects":
                        self.systemtray_icon.showMessage(
                            "Subjects", x.text() + " removed from the subject list"
                        )
                        self.systemtray_icon.hide()
                        logger.debug(
                            "Subjects-%s removed from the subject list.", x.text()
                        )
                        self.subjects[self.sem].remove(subject(x.text()))
                    else:
                        self.faculty_list_value.remove(x.text())
                        self.systemtray_icon.showMessage(
                            "Faculty", x.text() + " removed from the faculty list"
                        )
                        self.systemtray_icon.hide()
                        logger.debug(
                            "Faculty-%s removed from the faculty list.", x.text()
                        )
    ...
    def add_btn_elective_event(self):
        ...
        logger.debug("Adding %s elective to %s in %s",
                     text, sem, elective_group)
        if not sem:
            self.systemtray_icon.showMessage(
                "Input", "Please select the semester.")
            self.systemtray_icon.hide()
        ...
        if not short_sub:
            self.systemtray_icon.showMessage(
                "Input", "Please enter the elective short form."
            )
            self.systemtray_icon.hide()
        ...
        if not elective_group:
            self.systemtray_icon.showMessage(
                "Input", "Please select the elective group."
            )
            self.systemtray_icon.hide()
                ...
                for y in self.electives[sem][elective_group]:
                    if subj == y:
                        if text != "":
                            ...
                            y.subcode = subcode
                            logger.info(
                                "Elective details(name, shortname, credits, lab) :%s %s %s %s",
                                text,
                                short_sub,
                                credits,
                                lab,
                            ...
                            t = text + " - " + short_sub
                            self.ui_elec.elective_list.takeItem(
                                self.ui_elec.elective_list.row(x)
                            )
                            ...
                            self.systemtray_icon.showMessage(
                                "Input", "Please enter the elective name."
                            )
                            self.systemtray_icon.hide()
                            logger.debug(
                                "Input-Please enter the elective name.")
        ...
        if not row:
            ...
            for x in row:
                ...
                try:
                    ...
                    self.systemtray_icon.hide()
                    logger.debug(
                        "Subjects-%d removed from the subject list", x.text())
                    sub = subject(x.text())
            ...
            for group in self.electives[sem]:
                for y in self.electives[sem][group]:
                    if subj.name == y.name:
                        ...
                        self.ui_elec.elective_short_input.setText(
                            subj.short_name)
    ...
    def populate_second_window(self):
        ...
        for sem in self.sem_list:
            self.sections[sem] = list(map(chr, range(65, 65 + 26)))[
                : self.num_sections[sem]
            ]
    ...
    def section_combobox2_event(self):
        ...
        if sem and section:
            ...
            logger.info("Subject assignments: %s",
                        self.subjects_assigned[sem][section])
    ...
    def faculty_combobox2_event(self):
        ...
        logger.info("Faculty subject allotments: %s",
    ...
    def assign_btn_event(self):
        ...
        if faculty == "":
            self.systemtray_icon.showMessage(
                "Input", "Please select a faculty member.")
            self.systemtray_icon.hide()
        ...
        for i, x in enumerate(self.subjects_assigned[sem][section]):
            ...
            if subj == sub:
                try:
                    if (
                        self.subs[x[1]].lab == False or faculty in teachers
                    ):  # if theory subject or teacher already assigned to subject
                        self.systemtray_icon.showMessage(
    ...
    def undo_btn_event(self):
        ...
        if not selected:
            ...
            for x in selected:
                ...
                x = x.split(" - ")
                if (
                    x[2].split(" ")[0] not in self.sem_list
                ):  # deleting from section view
                    section = self.ui2.section_combobox.currentText()
                    sem = self.ui2.semester_combobox.currentText()
                    sub = x[0] + " - " + x[1]
                    f = x[2]
                    f = f.split(", ")
                    for faculty in f:
                        self.faculty_subjects[faculty].remove(
                            sub + " - " + sem + " " + section
                        )
                    faculty = ", ".join(f)
                    self.subjects_assigned[sem][section].remove(
                        sub + " - " + faculty)
                    pass
                    ...
                    pass
                logger.debug(
                    "Subject-faculty assignment: [%s - %s] removed", sub, faculty
                )
        pass

    # third form functions
    def semester_combobox3_event(
        self,
    ):  # load the number of sections in sec combobox and load the slot allotments made when semester is changed
    ...
    def section_combobox3_event(
        self,
    ):  # load the slot allotments made when section is changed
        self.ui3.subject_table.clearContents()
        sem = self.ui3.semester_combobox.currentText()
        section = self.ui3.section_combobox.currentText()
        ...
        if sem in self.section_fixed_slots and section in self.section_fixed_slots[sem]:
            logger.info(
                "Subject constraints: %s", self.section_fixed_slots[sem][section]
            )
            for row in self.section_fixed_slots[sem][section]:
                for column in self.section_fixed_slots[sem][section][row]:
                    logger.info("%s %s %s %s", sem, section, row, column)
    ...
    def cellClick3_event(self, row, column):
        ...
        slot = slot.split(
            " - "
        )  # slot is either subject - short name, Elective #, or -
        if len(slot) > 1:
            slot = slot[1]
        else:
            slot = slot[0]
        ...
        dict_entry_exists = (
            sem in self.section_fixed_slots
            and section in self.section_fixed_slots[sem]
            and row in self.section_fixed_slots[sem][section]
            and column in self.section_fixed_slots[sem][section][row]
        )
        ...
        if slot.startswith("Elective"):
            ...
            dict_entry_exists
            and self.section_fixed_slots[sem][section][row][column] == slot
        ):
            logger.info("%s exists", slot)
            self.section_fixed_slots[sem][section][row].pop(column)
            item.setText("")
    ...
    def cellClick4_event(self, row, column):
        ...
        faculty = self.ui4.faculty_combobox.currentText()
        if (
            faculty in self.faculty_fixed_slots
            and row in self.faculty_fixed_slots[faculty]
            and column in self.faculty_fixed_slots[faculty][row]
            and self.faculty_fixed_slots[faculty][row][column] == "-"
        ):
    ...
    def generate_event(self):
        ...
        self.timetables, self.faculty_timetables, dayclash = tt.produce_timetable(
            self, loc
        )
        self.section_combobox5_event()
    ...
    def section_combobox5_event(self):
        ...
        if sem in self.timetables and section in self.timetables[sem]:
            if self.timetables[sem][section].roomno:
                self.ui5.roomno_textbox.setText(
                    self.timetables[sem][section].roomno)
    ...
    def cellClick5_event(self, row, column):
        if self.selected_cell:
            logger.info(
                "Swapping alloted classes between two slots at user's risk")
            ...
            if r == row and c == column:  # if same cell is selected twice
                ...
                if (
                    self.ui5.inputType_combobox.currentText() == "Faculty"
                ):  # faculty timetable
                    faculty = self.ui5.faculty_combobox.currentText()
                    ...
                    if faculty in self.faculty_timetables:
                        ...
                        if cur_sub != "":
                            ...
                            cur_sec = sec[1]
                            logger.info(
                                "Swap element 1: %s %s %s", cur_sem, cur_sec, cur_sub
                            )
                            if self.timetables[cur_sem][cur_sec][prev_d][prev_t]:
                                ...
                                conflicts.append(
                                    (
                                        ...
                                        prev_d,
                                        prev_t,
                                        self.timetables[cur_sem][cur_sec][prev_d][
                                            prev_t
                                        ][3],
                                    )
                                )
                        ...
                        if prev_sub != "":
                            ...
                            prev_sem = sec[0]
                            prev_sec = sec[1]
                            prev_cell_text = "{} ({})".format(
                                prev_sub[1][3], prev_sem + " " + prev_sec
                            )
                            logger.info(
                                "Swap element 2: %s %s %s", prev_sem, prev_sec, prev_sub
                            )
                            if self.timetables[prev_sem][prev_sec][cur_d][cur_t]:
                                ...
                                conflicts.append(
                                    (
                                        ...
                                        cur_d,
                                        cur_t,
                                        self.timetables[prev_sem][prev_sec][cur_d][
                                            cur_t
                                        ][3],
                                    )
                                )
                        ...
                        if (
                            cur_sub
                            and prev_sub
                            and cur_sem == prev_sem
                            and cur_sec == prev_sec
                        ):
                            can_swap = True
                        ...
                        if can_swap:
                            ...
                            prev_cell.setText(prev_cell_text)
                            new_cell = QtWidgets.QTableWidgetItem()
                            new_cell.setText(cur_cell.text())
                            self.ui5.generated_table.setItem(r, c, new_cell)
                            self.ui5.generated_table.setItem(
                                row, column, prev_cell
                            )  # swap cell contents in table
                            f = self.faculty_timetables[faculty]
                            f[cur_d][cur_t], f[prev_d][prev_t] = (
                                f[prev_d][prev_t],
                                f[cur_d][cur_t],
                            )  # swap in faculty timetable
                            if prev_sub:
                                self.timetables[prev_sem][prev_sec][cur_d][cur_t] = (
                                    self.timetables[prev_sem][prev_sec][prev_d][prev_t]
                                )
                                self.timetables[prev_sem][prev_sec][prev_d][prev_t] = ""
                            if cur_sub:
                                self.timetables[cur_sem][cur_sec][prev_d][prev_t] = (
                    ...
                    if prev_sub != "":
                        ...
                        if self.subs[prev_sub[3]].lab == True:
                            ...
                            if self.faculty_timetables[prev_teacher][cur_d][cur_t]:
                                sec, sub = self.faculty_timetables[prev_teacher][cur_d][
                                    cur_t
                                ]
                                can_swap = False
                                conflicts.append(
                    ...
                    if cur_sub != "":
                        if self.subs[cur_sub[3]].lab == True:
                            ...
                            if self.faculty_timetables[cur_teacher][prev_d][prev_t]:
                                sec, sub = self.faculty_timetables[cur_teacher][prev_d][
                                    prev_t
                                ]
                                can_swap = False
                                conflicts.append(
                    ...
                    if can_swap:
                        ...
                        self.timetables[sem][section][prev_d][
                            prev_t
                        ] = cur_sub  # swap in section timetable

                        if prev_sub != "":
                            self.faculty_timetables[prev_teacher][cur_d][cur_t] = (
                                self.faculty_timetables[prev_teacher][prev_d][prev_t]
                            )
                            self.faculty_timetables[prev_teacher][prev_d][prev_t] = ""
                        if cur_sub != "":
                            self.faculty_timetables[cur_teacher][prev_d][prev_t] = (
    ...
    def roomno_textbox_event(self):
        logger.info(
            "Storing room number alloted to each section in each semester")
        sem = self.ui5.semester_combobox.currentText()
        ...
        if isinstance(self.timetables, dict):
                        ...
                        for day in tt:
                            for timeslot in tt[day]:
                                try:
                                    if (
                                        not tt[day][timeslot]
                                        or self.subs[tt[day][timeslot][3]].lab
                                    ):
            ...
            for day in rooms:
                for timeslot in rooms[day]:
                    if (
                        len(rooms[day][timeslot]
                            ) == num_rooms and num_rooms > 0
                    ):  # at times such as lunch breaks
    ...
    def print_btn_event(self):
        ...
        if inputType == "Students":
            ...
            if sem == "" or sec == "":
                self.systemtray_icon.showMessage(
                    "Warning!", "Select Semester and Section to print timetable."
                )
                self.systemtray_icon.hide()
                logger.debug(
                    "Warning! Select Semester and Section to print timetable.")
                ...
                filepath = os.path.join(
                    "Output", "Class Timetables", "{}.docx".format(tt.name)
                )
                try:
                    year = "{}. {} - {}. {}".format(
                        self.startMonth[:3],
                    ...
                    worddoc.make_docx(
                        ...
                        "section",
                        filepath,
                        self.subjects_assigned[sem][sec],
                        self.subs,
                        self.faculty_list_value,
                        year,
                    )
            ...
            if faculty == "":
                self.systemtray_icon.showMessage(
                    "Warning!", "Select the Faculty to print timetable"
                )
                self.systemtray_icon.hide()
                logger.debug("Warning! Select the faculty to print timetable.")
            else:
                tt = self.faculty_timetables[faculty]
                logger.info("Printing timetable for %s",
                            faculty_class(tt.name).name)
                filepath = os.path.join(
                    "Output",
                    "Personal Timetables",
                    "{}.docx".format(faculty_class(tt.name).name),
                )
                try:
    ...
    def publish_btn_event(self):
        ...
        if inputType == "Students":
            ...
            if sem == '' or sec == '':
                self.systemtray_icon.showMessage(
                    "Warning!", "Select Semester and Section to publish timetable into database."
                )
                self.systemtray_icon.hide()
                logger.debug(
                    "Warning!, Select Semester and Section to publish timetable into database."
                ...
                tt = self.timetables[sem][sec]
                logger.info(
                    "Publishing timetable into database for %s", tt.name)
                try:
                    db_publish = self.db.student_timetables
                    # Convert timetable
                    converted_data = transform_student_timetable_for_mongodb(
                        tt.items(), sem, sec, self.faculty_list_value
                    )

                    # Overwrite if document with same semester and section exists, else insert new
                    result = db_publish.replace_one(
                        {"semester": sem, "section": sec},  # Filter criteria
                        converted_data,                     # New document to insert or replace with
                        upsert=True                         # Insert if not found
                    )

                    # Optional: Log operation result
                    if result.matched_count > 0:
                        self.systemtray_icon.showMessage(
                            "Success", "Existing timetable updated."
                        )
                        logger.info("Existing timetable updated.")
                    else:
                        self.systemtray_icon.showMessage(
                            "Success", "New timetable inserted."
                        )
                        logger.info("New timetable inserted.")

                    # Optional: log or return the inserted ID
                    logger.info(
                        f"Inserted document IDs: {result.inserted_id}")
                except Exception as e:
                    logger.exception(e)

        elif inputType == "Faculty":
            logger.info("Faculty")
            faculty = self.ui5.faculty_combobox.currentText()
            if faculty == "":
                self.systemtray_icon.showMessage(
                    "Warning!", "Select the Faculty to publish timetable"
                )
                self.systemtray_icon.hide()
                logger.debug(
                    "Warning! Select the faculty to publish timetable.")
                ...
                tt = self.faculty_timetables[faculty]
                logger.info(tt)
                logger.info("Publishing timetable into database for %s",
                            faculty_class(tt.name).name)
                try:
                    i = self.faculty_list_value.index(faculty)
                    designation = self.faculty_list_value[i].designation
                    full_name = f"{self.faculty_list_value[i].title} {self.faculty_list_value[i].name}"
                    db_publish = self.db.timetable_techers
                    converted_data = transform_faculty_timetable_for_mongodb(
                        tt.items(), self.faculty_list_value)
                    result_faculty = db_publish.replace_one(
                        # Filter criteria
                        {"faculty": full_name, "designation": designation},
                        converted_data,                     # New document to insert or replace with
                        upsert=True                         # Insert if not found
                    )

                    # Optional: Log operation result
                    if result_faculty.matched_count > 0:
                        self.systemtray_icon.showMessage(
                            "Success", "Existing timetable updated."
                        )
                        logger.info("Existing timetable updated.")
                    ...
                    # Optional: log or return the inserted ID
                    logger.info(
                        f"Inserted document IDs: {result_faculty.inserted_id}")
                except Exception as e:
                    logger.exception(e)

        else:
            self.systemtray_icon.showMessage(
                "Warning!", "Select the Student or Faculty to publish timetable"
            )
            self.systemtray_icon.hide()
            logger.debug(
                "Warning!, Select the Student or Faculty to publish timetable"
    ...
    def show_printerror_dialog(self, error):
        ...
        msg.setIcon(QMessageBox.Critical)
        msg.setText(
            "There was an error with printing. Please close the docx file before printing it again."
        )
    ...
    def show_results_dialog(self, dayclash):
        ...
        msg.setInformativeText(
            "There were " + str(len(dayclash)) + " unallocated subjects."
        )
        info = "They are: \n" + "\n".join(
            map(lambda x: "{} {} {}".format(
                x[0].name, x[1][2], x[1][3]), dayclash)
        )
    ...
    def finish_btn_event(self, event):
        ...
        buttonReply = QMessageBox.warning(
            self,
            "EXIT Confirmation",
            "Are you sure you want to EXIT? All unsaved data will be lost on quitting. If you would like to save and quit, please click on Save.",
        ...
        if buttonReply == QMessageBox.Yes:
            ...
            if self.cur_open_filename:
                ...
                if success:
                    self.systemtray_icon.showMessage(
                        "Success", "Saved to " + self.cur_open_filename
                    )
                    self.systemtray_icon.hide()
                    logger.debug("Success. Saved to %s",
                                 self.cur_open_filename)
                else:
                    event.ignore()
            else:
    ...
    def next_btn_event(self):
        if self.YearWindow.isVisible():
            if (
                self.FirstWindow.isVisible()
                or self.SecondWindow.isVisible()
                or self.ThirdWindow.isVisible()
                or self.FourthWindow.isVisible()
                or self.FifthWindow.isVisible()
            ):  # if year window is opened from menu option
            ...
            if (
                self.ui4.faculty_combobox.currentText() == ""
                and self.ui4.faculty_combobox.count() == 0
            ):
                for faculty in self.faculty_list_value:
            ...
            if (
                self.ui5.faculty_combobox.currentText() == ""
                and self.ui5.faculty_combobox.count() == 0
            ):
                for faculty in self.faculty_list_value:
    ...
    def filemenuevent(self, option):
        ...
        if option == "Exit":
            ...
            buttonReply = QMessageBox.question(
                self,
                "Load Confirmation",
                "Are you sure you want to load data? All unsaved data will be lost on loading.",
            ...
            if buttonReply == QMessageBox.Ok:
                ...
                if dialog.exec_():
                    ...
                    if success:
                        self.systemtray_icon.showMessage(
                            "Success", "Loaded all saved data from " + fname
                        )
                        self.systemtray_icon.hide()
                        logger.debug(
                            "Success. Loaded all saved data from %s", fname)
                        logger.debug(
                            "Reset all windows and displaying 1st window")
                        ...
                        if (
                            self.SecondWindow.isVisible()
                            or self.ThirdWindow.isVisible()
                            or self.FourthWindow.isVisible()
                            or self.FifthWindow.isVisible()
                            or self.ElectiveWindow.isVisible()
                        ):
                            self.ElectiveWindow.hide()
                            self.SecondWindow.hide()
                            self.ThirdWindow.hide()
            ...
            buttonReply = QMessageBox.question(
                self,
                "Reset Confirmation",
                "Are you sure you want to clear all data? All unsaved data will be lost and the program will be reset.",
                QMessageBox.Ok | QMessageBox.Cancel,
                QMessageBox.Cancel,
            )
            if buttonReply == QMessageBox.Ok:
                ...
                self.systemtray_icon.showMessage(
                    "Reset", "All unsaved data have been cleared."
                )
                self.systemtray_icon.hide()
            ...
            try:
                ...
                for sem in self.timetables:
                    for sec in self.timetables[sem]:
                        tt = self.timetables[sem][sec]
                        filepath = os.path.join(
                            "Output", "Class Timetables", "{}.docx".format(
                                tt.name)
                        )
                        year = "{}. {} - {}. {}".format(
                            self.startMonth[:3],
                ...
                self.systemtray_icon.showMessage(
                    "Print All", "All timetables have been printed to " + fp
                )
                self.systemtray_icon.hide()
                logger.debug(
                    "Print All-All timetables have been printed to %s", fp)
                os.startfile(fp)
            except OSError as err:
                self.show_printerror_dialog(err)
                logger.exception(err)
    ...
    def update_sub(self):
        for sem in self.subjects:
            for i, sub in enumerate(self.subjects[sem]):
                self.subjects[sem][i] = subject(
                    sub.name, sub.short_name, sub.credits, sub.lab
                )

    def merge_data(self, faculty_list_value, subjects, electives):
        self.faculty_list_value = list(
            set(faculty_list_value).union(set(self.faculty_list_value))
        )
        self.faculty_list_value.sort()
        for sem in subjects:
            self.subjects[sem] = list(
                set(subjects[sem]).union(set(self.subjects[sem])))
        for sem in electives:
            n_groups = len(self.electives[sem])
            for group in electives[sem]:
                n_groups += 1
                self.electives[sem]["Elective " +
                                    str(n_groups)] = electives[sem][group]
    ...
    def load_state(self, fname):
        try:
            ...
            logger.info("Subjects assigned to faculty: %s",
                        self.faculty_subjects)
                ...
                for sec in sfs[sem]:
                    ...
                    for row in sfs[sem][sec]:
                        ...
                        for col in sfs[sem][sec][row]:
                            self.section_fixed_slots[sem][sec][int(row)][int(col)] = (
                                sfs[sem][sec][row][col]
                            )
            for teacher in ffs:
                ...
                for row in ffs[teacher]:
                    ...
                    for col in ffs[teacher][row]:
                        self.faculty_fixed_slots[teacher][int(row)][int(col)] = ffs[
                            teacher
                        ][row][col]
            return True
        except IOError as err:
        ...
        try:
            ...
            for r in range(1, rows):
                    ...
                    if not cur_sub or sub_name != cur_sub.name:
                        if not sub_short.upper().startswith("ELE"):
                            if lab:
                                ...
                                sub_short = " | ".join(
                                    map(str.strip, sub_short))
                            cur_sub = subject(
                                sub_name, sub_short, credits, lab, sub_code
                            )
                ...
                for i, fac_name in enumerate(f):
                    ...
                    faculty_subjects[fac.name].append(
                        "{} - {} - {} {}".format(sub_name,
                                                 sub_short, cur_sem, section)
                    )
                subjects_assigned[cur_sem][section].append(
                    "{} - {} - {}".format(sub_name, sub_short, ", ".join(f))
                )
...
if __name__ == "__main__":
    ...
    loc = (
        ...
        + os.path.join(
            "logs", time.strftime(
                "%a, %d %b %Y %H-%M-%S.log", time.localtime())
        )
    )
    my_logger()
